...
gboolean arguments_callback(const gchar *option_name,const gchar *value, gpointer data, GError **error){
  *error=NULL;
  ...
  if (!g_strcmp0(option_name, "--string-pk-planner")) {
    if (!string_pk_planner_strategy_from_string(value, &string_pk_planner_strategy)) {
      g_critical("--string-pk-planner accepts: auto, metadata, recursive");
    }
    g_free(string_pk_planner_strategy_str);
    string_pk_planner_strategy_str = g_strdup(string_pk_planner_strategy_name(string_pk_planner_strategy));
    return TRUE;
  }
  if (!g_strcmp0(option_name, "--string-pk-planner-timeout")) {
    if (value == NULL) {
      return FALSE;
    }
    string_pk_planner_timeout_seconds = (guint)g_ascii_strtoull(value, NULL, 10);
    return TRUE;
  }
  if (!g_strcmp0(option_name, "--string-pk-planner-max-probes")) {
    if (value == NULL) {
      return FALSE;
    }
    string_pk_planner_max_probes = (guint)g_ascii_strtoull(value, NULL, 10);
    return TRUE;
  }
  if (!g_strcmp0(option_name, "--string-pk-planner-max-prefixes")) {
    if (value == NULL) {
      return FALSE;
    }
    string_pk_planner_max_prefixes = (guint)g_ascii_strtoull(value, NULL, 10);
    return TRUE;
  }
  if (!g_strcmp0(option_name, "--string-pk-planner-min-rows")) {
    if (value == NULL) {
      return FALSE;
    }
    string_pk_planner_min_rows = g_ascii_strtoull(value, NULL, 10);
    return TRUE;
  }
  if (!g_strcmp0(option_name, "--string-pk-planner-target-rows-per-prefix")) {
    if (value == NULL) {
      return FALSE;
    }
    string_pk_planner_target_rows_per_prefix = g_ascii_strtoull(value, NULL, 10);
    return TRUE;
  }
...
static GOptionEntry chunks_entries[] = {
    ...
    {"string-pk-planner", 0, G_OPTION_FLAG_OPTIONAL_ARG, G_OPTION_ARG_CALLBACK, &arguments_callback,
      "Planner for string primary keys. Options: auto, metadata, recursive. Default: auto", NULL},
    {"string-pk-planner-timeout", 0, 0, G_OPTION_ARG_INT, &string_pk_planner_timeout_seconds,
      "Maximum planner time per table in seconds. Default: 30", NULL},
    {"string-pk-planner-max-probes", 0, 0, G_OPTION_ARG_INT, &string_pk_planner_max_probes,
      "Maximum number of planner probes per table. Default: 64", NULL},
    {"string-pk-planner-max-prefixes", 0, 0, G_OPTION_ARG_INT, &string_pk_planner_max_prefixes,
      "Maximum number of root prefixes generated by the metadata-assisted planner. Default: 256", NULL},
    {"string-pk-planner-min-rows", 0, 0, G_OPTION_ARG_INT, &string_pk_planner_min_rows,
      "Minimum estimated rows before auto planner attempts metadata-assisted prefix planning. Default: 1000000", NULL},
    {"string-pk-planner-target-rows-per-prefix", 0, 0, G_OPTION_ARG_CALLBACK, &arguments_callback,
      "Target rows per prefix chunk for the metadata-assisted planner. 0 (default) derives it from table rows / --string-pk-planner-max-prefixes; a positive value is used directly and the planner deepens prefixes until reached. Bounded by --max-char-size (depth) and --string-pk-planner-max-prefixes (count). Default: 0", NULL},
