...
guint string_pk_planner_max_depth=1;
...
gboolean arguments_callback(const gchar *option_name,const gchar *value, gpointer data, GError **error){
  *error=NULL;
  ...
  if (!g_strcmp0(option_name, "--string-pk-planner-max-depth")) {
    if (value == NULL) {
      return FALSE;
    }
    string_pk_planner_max_depth = (guint)g_ascii_strtoull(value, NULL, 10);
    return TRUE;
  }
...
static GOptionEntry chunks_entries[] = {
    ...
    {"string-pk-planner-max-depth", 0, 0, G_OPTION_ARG_INT, &string_pk_planner_max_depth,
      "Maximum prefix depth probed by the metadata-assisted planner. Default: 1", NULL},
